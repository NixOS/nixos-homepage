...
Unlike `.deb` and `.rpm` packages, which install into shared system directories, Nix puts each package or dependency into its own [dedicated path](https://nixos.org/guides/nix-pills/18-nix-store-paths.html) under `/nix/store/`. So when you fetch software from the cache, what you download is the compressed contents of a unique Nix store path: the directories and files that make up a specific package, dependency, or build artifact. The store path is _always_ unique because it includes a [hash derived from all the inputs](https://nixos.org/guides/nix-pills/18-nix-store-paths.html?highlight=hash#output-paths) used to build its contents. Nix prepends this input-addressed hash to the package's name and version, like so: `/nix/store/<hash>-cowsay-3.8.4`.

If this seems like madness, there's a method to it. By using input-addressed store paths, Nix guarantees a set of properties analogous in some respects to the ACID properties a relational database provides. Those properties are what give you reproducible build- and run-time behavior, reliable rollbacks, automatic garbage collection, and safe multi-user installs.

Nix can automatically reclaim disk space once store objects (packages, build inputs, etc.) are no longer referenced by the global system or by individual users' profiles. Multiple users can also install, upgrade, or remove packages on the same system at the same time, including packages that pull in _conflicting versions of the same dependencies_. For instance, say you need to run `kubectl` 1.27 (built from an older package set that uses glibc 2.35) alongside Git 2.51 (built from a newer package set that uses glibc 2.40): thanks to Nix's hashed, input-addressed store paths, _both_ CLIs can coexist peacefully on the same machine.
